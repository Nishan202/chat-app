...
class FirebaseRepository {
  ...
  Future<void> createUser({required UserModel user, required String password}) async {
    try{
      var userCredential = await firebaseAuth.createUserWithEmailAndPassword(email: user.email!, password: password);
      if(userCredential.user != null){
        firebaseFirestore.collection(COLLECTION_USERS).doc(userCredential.user!.uid).set(user.toDoc()).catchError((error){
